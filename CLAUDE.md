...
6. **`meta.updated`** is the single "Last updated" source. Bump it on every content change; preflight fails the deploy if `content.ts` changed and it did not (unless it already reads today, ±1 day for the UTC+3 owner and the UTC runner: a second change on the same day). Do not derive it from `Date.now()`.
...
| `guestbook` | entries from `src/content/guestbook.json` (each with its `lang`) + a small form (name, message, hidden honeypot, `echo` button; client only, only while `guestbook.endpoint` is set) |
...
CONTENT = {
  ...
  guestbook: { endpoint },   // entries live in src/content/guestbook.json: [{ id, name, date, lang, message }], written by the Worker
  contact: { note:{en,tr} },
  colophon: { en:[...], tr:[...] },
}
...
Guestbook: `echo name: message` or the form. The entry appears immediately with "awaiting review" and `engine.postGuest()` posts `{ name, message, hp, lang, t }` (t = ms since page load) to `guestEndpoint` (resolved by `boot()`: `?guestbook=<url>` or `guestbook.endpoint`). The Worker (`ask/guestbook.mjs`, §9.4) filters, moderates with DeepSeek, commits approved entries to `src/content/guestbook.json` — the deploy that commit triggers publishes them, a few minutes later — and emails the owner for every entry with a signed Publish (held) or Remove (approved) link. The terminal says `→ signed — it appears on the page in a few minutes` or `→ thanks — I read this one before it goes up`. `guestbook.json` is visitor data, not owner copy: never edit it by hand except to remove an entry, and keep one entry per line.
...
Hero line 2 and availability · Now paragraph · About · TAZI bullets (3) · thesis line · third experience entry (or delete) · projects churn/toolbench/kvstore/survey copy, write-ups and links · writing (2) · reading (4) · email, GitHub, LinkedIn, CV URL · `meta.ask` / `meta.mail` endpoint · contact note · confirm skills lists.
...
`ask/` holds a Cloudflare Worker (`worker.mjs`) that answers the "ask me a question" field: it checks the `Origin` against `ALLOWED_ORIGINS`, throttles per IP (5 a minute, optional binding), caps the question at 300 characters, calls **DeepSeek** through `ask/llm.mjs` (the one model client: `POST {LLM_BASE}/chat/completions`, `MODEL` default `deepseek-flash` = DeepSeek-V4.1-Flash, key `DEEPSEEK_API_KEY`; thinking mode is turned **off** on every call, so text streams at once and `temperature` applies; only `delta.content` is passed on, never `reasoning_content`) — answers at temperature 0.7, 600 tokens, streaming, with an extra rule for plain text (no headings, tables or code blocks, bold at most once); debate utterances at 1.0 with a plain-text rule that keeps the `== ` / `?? ` marker lines — with the last four turns of `history` as alternating messages, `knowledge.txt` as the knowledge base (the site map of section ids, project slugs and safe commands, then every knowledge file under a `=== id | title | href` header) plus fixed rules (first person, only from the files, say when unsure, under 120 words, never phone numbers, at most two `[[go|project|cmd:…]]` tokens, up to two `?? ` follow-ups), and streams the text back as `text/plain`. With `mode: 'debate'` it runs the debate of §5.14 instead (`debate.mjs`, `verify.mjs`, `characters.json`; `npm run ask:test` runs their unit tests). **Knowledge files:** `npm run ask:prompt` (`build-prompt.mjs`) generates `ask/knowledge/profile.md` from the English `CONTENT` (placeholders skipped, `[[tokens]]` flattened) and concatenates it with every other `ask/knowledge/*.md` — the owner's CV, project write-ups, code notes, each with front matter `id` (lowercase ASCII, the citation key), `title` (the chip label) and optional `href` (a `#section`/`#projects/<slug>` hash or a URL a reader can check) — into `ask/knowledge.txt`; it **fails** on a `Placeholder:` and on anything shaped like a phone number (nine digits or more), naming the file and line. Preflight notes when `knowledge.txt` is older than `content.ts` or a knowledge file. Deploy: `wrangler login`, `npm run ask:prompt`, `wrangler secret put DEEPSEEK_API_KEY`, `wrangler deploy` (all in `ask/README.md`), then set `meta.ask` and bump `meta.updated`. **The API key is a Worker secret and never appears in this repo**, which is public. The same Worker answers `POST /mail` (§5.13): honeypot and a 3s floor (accepted and dropped), its own `MAIL_LIMITER` (3 a minute per IP), address/length checks, CR/LF stripped, then Resend's REST API (`RESEND_API_KEY` secret) from `MAIL_FROM` (`onboarding@resend.dev` until a domain is verified, which only delivers to the Resend account's own address, hence `MAIL_TO`) with `reply_to` = the visitor. Setup in `ask/README.md` → Mail; with a domain, SPF/DKIM at the registrar and a new `MAIL_FROM` (§9.1). The same Worker answers **`POST /guestbook`** (`ask/guestbook.mjs`, tests in `guestbook.test.mjs`): honeypot and the 3s floor (accepted and dropped), `GUEST_LIMITER` (3 a minute per IP), name ≤ 40 and message 2–300 characters, then `prefilter()` (an email address, a link or a phone-like number holds the entry without a model call), then the model (`llm.complete()`, JSON mode `response_format: json_object` with an example in the prompt, temperature 0; `{decision: approve|hold, reason}`, the entry passed as untrusted data inside `<entry>` tags; a `content_filter` finish, an API error, a missing key, empty content or anything that is not exactly that JSON **holds**). Approved entries are committed through the GitHub contents API (`GITHUB_REPO` var, `GITHUB_TOKEN` secret: a fine-grained PAT for this repo only, Contents read/write) as `content: guestbook entry from <name>`, which triggers the deploy. Every entry is emailed to `MAIL_TO` with the verdict and reason and a link to **`/guestbook/admin`**: the link carries the entry, HMAC-signed with `GUESTBOOK_SECRET` and valid 30 days (no storage); GET only shows a confirmation page (mail scanners follow links), the POST from its button publishes or removes. That route skips the origin check — the signature is its guard. Nothing in `ask/` is built or served by Pages.
