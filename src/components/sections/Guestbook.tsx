...
/* Entries plus a small form.  A new entry shows at once as "awaiting review";
   publishing means copying an approved issue into CONTENT.guestbook.entries. */
export function Guestbook({ def }: { def: PlatedDef }) {
  const engine = useEngine();
  const { pendingGuests } = useDoc();
  const { L, ui } = useLang();
  ...
  const entries = [
    ...pendingGuests,
    ...CONTENT.guestbook.entries.filter(g => shown(g.name)).map(g => ({ name: g.name, date: g.date, message: L(g.message), pending: false })),
  ];
  const submit = (e: FormEvent) => {
    e.preventDefault();
    engine.postGuest(name.trim(), message.trim());
    setName(''); setMessage('');
  };
  return (
    <Section def={def}>
